...
class PaperWork extends React.Component {
    ...
    try {
      const thumbnailRef = myStorage
        ...
        .child(
          `${
            this.transactionID
          }/paperworks/thumbnails/${getEffectiveDocumentName(docName)}.png`
        );
  ...
  async setDocumentList() {
    let paperworkDataSnapshot = await myFirestore
    ...
    paperworkDataSnapshot.docs.map((doc) => {
      ...
      if (
        ...
        let documentRef = myStorage
          .ref()
          .child(`${this.transactionID}/paperworks/${paperworkMeta.path}`);

        let creator =
          paperworkMeta.creator === getCurrentUser().email
            ? "You"
            : peopleList.filter(
                (person) => person.email === paperworkMeta.creator
              )[0]
            ? peopleList.filter(
                (person) => person.email === paperworkMeta.creator
              )[0].name
      ...
      return (
        <div className="doc-card-group">
          {this.state.documents.map((docData, itemIndex) => (
            ...
            >
              <div className="doc-card-root" key={docData.name}>
                <IconButton
          ...
          <Modal
            ...
            dismissCallback={() =>
              this.setState({ isPaperworkExistsModalVisible: false })
            }
            modalWidth={700}
          >
            <Box
            ...
            >
              ...
              <div style={{ fontSize: 18 }}>
                ...
                upload this paperwork, delete the existing paperwork first.
              </div>
            </Box>
          ...
          <AccessRightsModal
            ...
            dismissCallback={() =>
              this.setState({ isAccessRightsModalVisible: false })
            }
