        ...
        myFirestore
          ...
          .then((doc) => {
            if (!doc.exists) {
              ...
              myFirestore
                .doc(`users/${localStorage.getItem("userID")}`)
                .set(Object.assign(user, { transactions: [] }));
            } else {
              myFirestore
                .doc(`users/${localStorage.getItem("userID")}`)
                .update(user);
          ...
          .then(() => {
            myFirebase.auth().currentUser.sendEmailVerification(); // sending the email for verification
...
export function login(user) {
  return (dispatch) => {
    ...
    myFirebase
      .auth()
      .signInWithEmailAndPassword(user.email, user.password) // Logging in the user
      .then((res) => {
...
export function facebookAuth() {
  return (dispatch) => {
    myFirebase
      .auth()
      .signInWithPopup(new myFirebase.auth.FacebookAuthProvider())
      .then((res) => {
        localStorage.setItem("userID", res.user.uid); // storing the userID in the localstorage
        res.user
          .getIdToken()
    ...
    myFirestore
      ...
      .then((doc) => {
        if (!doc.exists) {
          ...
          myFirestore
            .doc(`users/${localStorage.getItem("userID")}`)
            .set(Object.assign(info, { transactions: [] }));
        } else {
          myFirestore
            .doc(`users/${localStorage.getItem("userID")}`)
            .update(info);
...
export function editingUser(newUser) {
  // to edit the current user
  return (dispatch) => {
    myFirestore
      .doc(`users/${localStorage.getItem("userID")}`)
      .update(newUser)
      ...
      .catch((err) => {
        if (err.response) {
          ...
          dispatch(
            setErrors({ error: "Please check your internet connection" })
          );
        }
      });
  };
  ...
  return (dispatch) => {
    ...
    myFirebase
      ...
      .then((token) => {
        ...
        axios
          .post(`/invite/${transId}`, null, {
            // Adding the signed in user to the transaction
            headers: { Authorization: "Bearer " + token },
          })
          .then(() => {
            axios
              .put(`/add-transaction-to-user/${transId}`, null, {
                headers: { Authorization: "Bearer " + token },
              })
              .then(() => {
  ...
  return (dispatch) => {
    ...
    myFirebase
      ...
      .then((token) => {
        ...
        axios
          .post(`/add-user-details/${localStorage.getItem("userID")}`, user)
          .then(() => {
...
export function addUser(
  ...
  // User is added to the redux store
  return {
    type: ADD_USER,
    id: id,
    firstName,
